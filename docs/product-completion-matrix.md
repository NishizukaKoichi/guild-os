...
| Branch and HEAD | `main` at pushed evidence commit `774f0419a38cbafbe93b15ebb2b70a0a656816dc` before this matrix update; exact staging and restore source remains `63de2470e6ec7e5c4783ed947e54de7c78534eee` |
| Remote divergence | None at audit start; Core `main` and `origin/main` both pointed to `774f0419a38cbafbe93b15ebb2b70a0a656816dc` |
| Origin | `origin/main` contains the evidence record and exact staging/restore source commit; this matrix update is local until explicitly pushed |
...
| Hosted CI | **Blocked by external account state**: Core run `32485703267` at `774f0419a38cbafbe93b15ebb2b70a0a656816dc` and Distribution run `32479510084` at `f851b8870a9a7f7d8526214c68aee418fa4152df` both failed with zero executed steps. GitHub's exact annotation says recent account payments failed or the spending limit must be increased. Local equivalent gates are green; changing billing is an owner-only external action. |
| Distribution repository | Separate Git root `/Volumes/Pensive/Workspace/NishizukaKoichi/guild-os-distribution`; local commit `92bfef91d27e2ea6a35b20cf41439068916c2869` adds encrypted signing-key custody and is one commit ahead of `origin/main` at `f851b8870a9a7f7d8526214c68aee418fa4152df`. Both pin Core `63de2470e6ec7e5c4783ed947e54de7c78534eee`. |
...
| Release signing custody | Distribution commit `92bfef91d27e2ea6a35b20cf41439068916c2869` implements separate encrypted Ed25519 release and entitlement keys, owner-only files outside Git, two signed offline-backup attestations, named Human activation, dual-signed public trust state, active-only Installer/Updater acquisition, retired-key historical audit, and unconditional revoked-key denial. Twenty Distribution tests plus build, lint, compliance, SBOM, dependency audit, and deterministic clean-room rehearsal pass locally. A production key set is not operationally active until two real offline storage boundaries and the named Human approval exist. |
...
| 37 | Safe implementation sequence | In progress | Core and Distribution implementation, local gates, isolated owner-controlled staging, exact-release evidence, Access smoke, Human initialization, signed Distribution, real-account Installer preflight, isolated full-store restore, and signing-custody implementation are complete. The current signing-custody and matrix commits remain local until explicit push; fresh independent-account Installer/Updater rehearsal remains. |
| 38 | Required test coverage | Implemented but unverified | Builder, Personal, profiles, Space context, Ask/Plan/Act, permissions, Distribution, expiry, rollback, restore preparation, signing custody, i18n, mobile, accessibility, and all PostgreSQL/Gatekeeper integration tests pass. Real Access-protected cloud smoke, Personal initialization, Installer preflight, and full-store restore also pass; fresh-account clean-room Installer/Updater and operational key activation remain. |
| 39 | Technical completion | Missing | Independent-account clean-room install/update, hosted CI execution, activation of a production signing set against two real offline storage boundaries, and other rows marked unverified remain. The signing-custody implementation itself is complete and locally verified. |
...
| 41 | Final evidence report | Implemented but unverified | Exact Core and Distribution commits, signed manifest, isolated staging resource/version evidence, Runtime-role database verification, Access smoke, Human initialization, Installer preflight, encrypted backup, isolated full-store restore, Break Glass recovery, RPO/RTO, cleanup, and local signing-custody test evidence are recorded. Hosted CI is externally billing-blocked; fresh-account installation/update and operational activation of the production signing set remain. |
