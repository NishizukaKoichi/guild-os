...
| Open Core access | The Apache Core can be obtained without seller approval and self-installed by a technical user while preserving exact license and notice material. Before publication, all reachable Core Git history is scanned with a pinned scanner; every ignored fingerprint is bound to a reviewed synthetic-test line and any unreviewed finding fails closed | Machine-generated full-history publication audit with zero unreviewed findings; credential-free HTTPS clone; exact Core and Cloudflare OS checkout; strict Git object verification; frozen install; typecheck; build; source/license/notice checksums; and a commercial-readiness re-execution of the complete anonymous acquisition |
| Owned Distribution | A separate `guild-os-distribution` repository pins an exact Apache Core release and contains the official Installer, Updater, signed bundle, diagnostics, entitlement, compliance outputs, legal drafts, and handover material without importing private code into Core. Release v2 contains clonable local Git source archives for the exact Core and pinned Cloudflare OS commits; the signed manifest hashes both archives, executable CLI launchers, Distribution source/tools, lockfile, notices, SBOM, and migrations. Acquired packages never require a seller source repository. | Separate Git root, boundary tests, exact Core pin, full-clone and strict-object checks, source-archive checkout test, executable-mode and acquired-package CLI smoke, manifest tamper test, package tests, bundle manifest, artifact inventory |
| Installer | A purchaser can preflight, configure, dry-run, migrate, deploy, authenticated-smoke, and receive recovery/handover evidence entirely in purchaser-owned infrastructure without sending Secrets to the seller. The purchaser owns the Access application and smoke Service Token; the Installer verifies those exact objects and creates only exact Human administrator and token-specific Service Auth policies. It checks out Core and Cloudflare OS only from the signed local source archives, retains them under purchaser control, and records manifest-bound hashes. Explicit Human Root initialization remains first run, not an automated credential bypass. | Deterministic clean-room run, independent purchaser installation, Secret-leak test, no-network source test, Cloudflare REST contract, exact Access-policy test, failure cleanup, real smoke evidence hash, v4 installation evidence, installed state v3, sanitized handover report |
| Updater and entitlement | Signed exact-commit releases update in place only after compatibility, verification of the archived installed source, an old-Core backup, candidate local-source archive verification, migration dry-run and preflight, backward-compatible additive migration, candidate build/deploy, authenticated smoke, and rollback preparation. Deployment lock, source hashes, and Worker Version IDs remain exact. No seller source fetch is allowed. Entitlement expiry blocks new downloads only and never affects installed runtime or export | Signature/source-tamper tests, no-network source test, expiry runtime test, successful v3 update, failed-smoke Worker rollback, retained-schema evidence, release evidence |
...
| Commercial operations | Export, encrypted backup, isolated restore, customer-owned migration, support boundary, and optional Care are documented and operable without perpetual seller labor. Restore proof must be generated from live pre-recovery PostgreSQL/KV/R2/smoke comparison and post-recovery Human Root/Chronicle/smoke observation; a hand-authored status document is not evidence | Clean-room handover, two-phase generated backup/restore rehearsal, commercial draft docs, diagnostics |
...
| Operations | Purchaser-owned deploy, model/provider configuration, observability, export, backup, restore, rollback, and administrator handover. Existing deployments can transactionally separate a legacy Runtime schema owner and preserve already-provisioned Cloudflare Secret bindings without reading or rotating their values | Role-separation transaction tests, existing-Secret name preflight tests, clean-room setup, checksummed backup, isolated restore, release evidence |

The commercial readiness verifier is part of this acceptance boundary. Readiness request/report v2
must reject a missing, hand-authored-only, stale, private, or credential-dependent Open Core
acquisition by re-executing the complete anonymous clone/install/build. It must also reject synthetic
or injected Installer/Updater evidence, seller-owned Cloudflare accounts, owner-only restore
preparation, inactive signing custody, draft professional approvals, stale CI, and evidence from a
different commit. It must also reject evidence that requires seller-network source acquisition or
whose candidate source hashes differ from the exact signed manifest. Passing local Core or
Distribution tests is necessary but cannot satisfy those external gates.
