...
BEGIN
  ...
  -- A copied link retains the immutable security boundary from its original upload.
  IF EXISTS (
    SELECT 1 FROM knowledge_version_files
     WHERE guild_id = NEW.guild_id AND file_id = NEW.file_id
       AND knowledge_id = NEW.knowledge_id
  ...
  IF file_row.space_id IS DISTINCT FROM knowledge_row.space_id
    ...
    RAISE EXCEPTION 'New Knowledge file security boundary must match its Knowledge record';
  END IF;
  RETURN NEW;
END;
$$;
