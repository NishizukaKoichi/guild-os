...
test("keeps management out of the default member navigation", async ({ page }) => {
  ...
  await expect(page.locator(".primary-nav [data-app-page='members']")).toHaveCount(0);
  await expect(page.locator(".primary-nav [data-app-page='memory']")).toBeVisible();
  await expect(page.locator(".primary-nav [data-app-page='activity']")).toBeVisible();
  await expect(page.getByRole("button", { name: "AI agents", exact: true })).toHaveCount(0);
...
test("uses the six-item canonical mobile tab bar without horizontal overflow", async ({ page }) => {
  const errors = collectBrowserErrors(page);
  ...
  await page.locator(".nav-section-more > .nav-group-toggle").click();
